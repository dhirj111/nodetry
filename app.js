...
app1.use('/add-product', (req1, res1, next1) => {
  ...
  res1.send("<h1> the add payment page <h1>");
  next1();
  //jumps to next middleware
});

app1.use('/',(req1, res1, next1) => {
  //remember '/' is route which every route contains like  /about /addproduct
  //so if u will use before other custom routes it will execute / and then jump to next()[if invoked]
  //so it will execute for every route apart from above custome route middleware
